...
def getitem(M, k):
    ...
    return M.f[k[0],k[1]] if k in M.f else 0
...
def equal(A, B):
    ...
    #(d0,d1) for d0 in A.D[0] for d1 in A.D[1] if getitem(A,(d0,d1))!=getitem(B,(d0,d1))
...
def add(A, B):
    ...
    """
    assert A.D == B.D
    return Mat((A.D[0], A.D[1]),{(d0,d1):getitem(A,(d0,d1))+getitem(B,(d0,d1)) for d0 in A.D[0] for d1 in A.D[1]})
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat((M.D[0], M.D[1]),{(d0,d1):x*value for (d0,d1), value in M.f.items()}) #Preserves sparsity
...
def transpose(M):
    ...
    True
    """
    return Mat((M.D[1], M.D[0]),{(d1,d0):value for (d0,d1), value in M.f.items()}) #Preserves sparsity
...
def vector_matrix_mul(v, M):
    ...
    """
    assert M.D[0] == v.D
    return Vec(M.D[1], {k:sum([v[l]*getitem(M,(l,k)) for l in M.D[0]]) for k in M.D[1]}) #First creating a list of each column vector multiplied by row vector and then summed to get dot product. Did that to each column of the matrix to get a final vector which has the domain of the columns of the original matrix 
...
def matrix_vector_mul(M, v):
    ...
    """
    assert M.D[1] == v.D
    return Vec(M.D[0], {k:sum([v[l]*getitem(M,(k,l)) for l in M.D[1]]) for k in M.D[0]})
...
def matrix_matrix_mul(A, B):
    ...
    """
    assert A.D[1] == B.D[0]
    return Mat((A.D[0], B.D[1]), {(d0,d1):sum([getitem(A,(d0,l))*getitem(B,(l,d1)) for l in A.D[1]]) for d0 in A.D[0] for d1 in B.D[1]} )
    #Doing a (dot product of col vector of A * row vector of B) for each row of A and column of B
